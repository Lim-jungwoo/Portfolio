...
namespace ft
{
	...
	class vector
	{
		...
		public:
			//constructor 사이즈가 0이면 어떻게 되는지 확인해봐야 한다.
				...
				_alloc(alloc), _start(NULL), _size(NULL), _capacity(NULL)
			{
				...
				{
					this->start = this->_alloc.allocate(n);
			...
			iterator	erase(iterator first, iterator last)
			{
				...
				for (size_type i = 0; i < this->_size - &(*last); i++)
				{
			...
			iterator	insert(iterator position, const value_type& val)
			{
				...
				if (this->capacity() >= this->size() + 1)
				{
					for (size_type i = 0; i < back_part; i++)
					{
						value_type	tmp = *(this->_size - i - 1);
						this->_alloc.construct(this->_size - i, tmp);
				...
				{
					pointer	new_start;
					size_type	capacity_size = (this->capacity() > 0) ? capacity() * 2 : 1;
			...
			void	insert(iterator position, size_type n, const value_type& val)
			{
				if (n == 0)
					return ;
				if (n + this->size() > this->max_size())
					throw (std::length_error("vector"));
				size_type	front_part = &(*position) - this->_start;
				if (this->capacity() - this->size() >= n)
				{
					for (size_type i = 0; i < this->size() - front_part; i++)
				...
				typename ft::enable_if<!ft::is_integral<InputIterator>::value, InputIterator>::type* = NULL)
			{
				...
				else
				{
					...
					for (; insert_position < this->size(); insert_position++)
						this->_alloc.construct(new_start + insert_size + insert_position, *(this->_start + insert_position));
					for (size_type i = 0; i < this->size(); i++)
						this->_alloc.destroy(this->_start + i);
					this->_alloc.deallocate(this->_start, this->capacity());
					this->_start = new_start;
			...
			void	reserve(size_type n)
			{
				...
				else
				{
					...
					if (this->_start)
						this->_alloc.deallocate(this->_start, this->capacity());
					this->_start = new_start;
					this->_size = new_start + this->size();
